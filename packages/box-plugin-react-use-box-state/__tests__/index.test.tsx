...
import { createBox, Base } from "@hobox/core"
import { useBoxState } from "../src"
...
describe("nestedDataChange", () => {
  ...
  const TodoList = () => {
    todosRenderCount += 1
    const [todoList, setTodoList] = useBoxState(todosBox)
    return (
      <ul>
  ...
  const FirstTodo = () => {
    ...
    const [firstTodo] = useBoxState(todosBox, (d) => d[0])
    ...
    return (
      <>
        <button
          ...
          onClick={() => {
            const firstTodoText = todosBox?.getData()?.[0]?.text
